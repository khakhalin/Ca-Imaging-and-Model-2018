...
randomizePositions = 0;             % Set to 1 if all xy positions should be randomly reassigned
...
doCorrelationFig = 0;               % plot raw correlation matrices
...
doResponseDynamics = 0;             % Early sells, late cells
...
for(iBrain = 1:nBrains)
    ...
    if(doPCA)
        ...
        if(1) % Rotation
            ...
            if(latency(scores(:,1))>latency(scores(:,2)))   % First comoponent should have shorter latency; if it doesn't - swap
...
%     if(0)                                       % Compare full response-based selectivity to peak-based selectivity
%         figure; subplot(1,2,1); plot(amps(:),peaks(:),'.'); title(name); xlabel('Full amplitude'); ylabel('Peak amplitude');
%         subplot(1,2,2); plot(selFC,selFCp,'.'); xlabel('Selectivity full amp'); ylabel('Selectivity on peak amp');
%         drawnow();   % Are peak and cumulative amplitudes different? Yes they are.
%     end

    saveResults(auxFolder,folderName{iBrain},'selC',selC);
    ...
    if(doResponseDynamics)
        ...
        lat = zeros(nCells,1);                          % Future latencies
        for(iCell=1:nCells)
        ...
        if(pcaStimType==3)
            cSel = selFS(:);                            % If we look at S, calculate everyting for S as if it were C
        else
            cSel = selFC(:);                            % Otherwise, default (C)
        end
        [r1,pval1] = corr(lat(:),cSel(:),'Type','Spearman');                % Correlation: early asymmetry rank to FC selectivity
        [r2,pval2] = corr(dist(:),lat(:),'Type','Spearman');                % Correlation latency vs distance from the origin point
        [r3,pval3] = corr(dist(:),cSel(:),'Type','Spearman');               % Correlation selectivity vs distance from the origin point
        [glmCoeffs,glmDev,glmStats] = glmfit([dist(:) xy(:,1)],cSel(:));
        glmREstimationDist = glmStats.t(2)/sqrt(nCells-2+glmStats.t(2)^2);
        glmREstimationX    = glmStats.t(3)/sqrt(nCells-2+glmStats.t(3)^2);
        if(iBrain==1); fprintf('    Name     Lat vs Sel       Dist vs Lat    Dist vs Sel     Origin point xy     Multivariate regression\n'); end
