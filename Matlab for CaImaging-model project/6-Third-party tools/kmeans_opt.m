function [IDX,VarShareExplained,C,SUMD,K]=kmeans_opt(X,varargin)
%%% [IDX,VarExplained,C,SUMD,K]=kmeans_opt(X,varargin) returns the output of the k-means
...
% By Sebastien De Landtsheer
% https://www.mathworks.com/matlabcentral/fileexchange/65823-kmeans_opt-optimal-k-means-with-elbow-method?focused=8772533&tab=function

%%% sebastien.delandtsheer@uni.lu
%%% sebdelandtsheer@gmail.com
%%% Thomas.sauter@uni.lu

% Aug 07 2018: updated to calculate VarShareExplained
...
[i,~]=find(PC>Cutoff,1,'first'); %find the best index
K=1+i; %get the optimal number of clusters
[IDX,C,SUMD]=kmeans(X,K); %now rerun one last time with the optimal number of clusters
VarShareExplained = sum(SUMD)/sum(sum(bsxfun(@plus,X,-mean(X)).^2));
